...
import com.liuhf.pan.core.utils.UUIDUtil;
...
public class ScheduleManager {
    ...
    public void stopTask(String key) {
        ...
        if (cancel) {
            log.info("{} 停止成功! 唯一表示为: {}", holder.getScheduleTask().getName(), key);
        } else {
            log.error("{} 停止失败! 唯一表示为: {}", holder.getScheduleTask().getName(), key);
        }
    }
    ...
    public String changeTask(String key, String cron) {
        ...
        ScheduleTaskHolder holder = cache.get(key);
        if (Objects.isNull(holder)) {
            throw new RPanFrameworkException("唯一表示不存在");
        }
        stopTask(key);
